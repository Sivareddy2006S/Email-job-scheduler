...
export default function SentEmailsTable({ refreshKey }: { refreshKey: number }) {
  ...
  const load = async () => {
    ...
    try {
      if (searchQuery.trim()) {
        ...
        // Search API currently returns all matches for the user, filter to sent/failed locally
        const sentItems = data.items.filter(i => i.status === 'sent' || i.status === 'failed');
        setItems(sentItems);
        setTotal(sentItems.length);
        setPage(1);
      } else {
        const data = await emailApi.listSent(page);
        setItems(data.items);
        setTotal(data.total);
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white">
      ...
      {!loading && !error && items.length === 0 && (
        <div className="p-6 text-sm text-slate-500">No sent emails found.</div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left text-slate-500">
                  ...
                  <th className="px-4 py-2 font-medium">Sent Time</th>
